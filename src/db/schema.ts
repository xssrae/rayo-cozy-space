...
export const focusPreset = pgEnum("focus_preset", [
  "classic",
  "deep_work",
  "custom",
]);
...
export const tasks = pgTable(
  ...
  (table) => [
    ...
    uniqueIndex("tasks_id_project_workspace_unique").on(
      table.id,
      table.projectId,
      table.workspaceId,
    ),
...
export const skills = pgTable(
  ...
  (table) => [
    uniqueIndex("skills_active_name_unique")
...
export const focusSessions = pgTable(
  ...
  (table) => [
    uniqueIndex("one_live_focus_per_workspace")
    ...
    foreignKey({
      columns: [table.projectId, table.workspaceId],
      foreignColumns: [projects.id, projects.workspaceId],
      name: "focus_project_workspace_fk",
    }).onDelete("restrict"),
    foreignKey({
      columns: [table.taskId, table.projectId, table.workspaceId],
      foreignColumns: [tasks.id, tasks.projectId, tasks.workspaceId],
      name: "focus_task_project_workspace_fk",
    }).onDelete("restrict"),
  ],
);
...
export const focusSessionSkills = pgTable(
  "focus_session_skills",
  {
    focusSessionId: uuid("focus_session_id")
      .notNull()
      .references(() => focusSessions.id, { onDelete: "cascade" }),
...
export const focusSessionTags = pgTable(
  "focus_session_tags",
  {
    focusSessionId: uuid("focus_session_id")
      .notNull()
...
export const workspaceImports = pgTable(
  ...
  (table) => [
    uniqueIndex("workspace_import_fingerprint_unique").on(
      table.workspaceId,
      table.fingerprint,
    ),
  ],
