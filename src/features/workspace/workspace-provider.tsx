...
export function WorkspaceProvider({ children }: { children: ReactNode }) {
  ...
  const value: WorkspaceContextValue = {
    ...
    addProject: async (project) =>
      run(() =>
        saveProjectFn({
          data: { ...project, dueDate: project.dueDate ?? null },
        }),
      ),
    ...
    deleteProject: async (projectId) =>
      run(() => archiveProjectFn({ data: { id: projectId } })),
    addTask: async (task) => {
      const id = await saveTaskFn({
        data: { ...task, dueDate: task.dueDate ?? null },
      });
      await refresh();
      return id;
    },
    updateTask: async (task) =>
      run(() =>
        saveTaskFn({ data: { ...task, dueDate: task.dueDate ?? null } }),
      ),
    toggleTaskDone: async (taskId) =>
      run(() => toggleTaskFn({ data: { id: taskId } })),
    ...
    importWorkspace: async (contents) => {
      try {
        await importWorkspaceFn({ data: { contents, sourceVersion: "file" } });
        await refresh();
        return true;
      } catch {
        return false;
